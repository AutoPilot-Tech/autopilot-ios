    ...
    var body: some View {
        VStack {
            TabView(selection: $selectedTab) {
                ForEach(0 ..< viewModel.routine.count) { index in
                    VStack(spacing: 0) {
                        if let url = Bundle.main.url(forResource: viewModel.routine[index].videoName, withExtension: "mp4") {
       ...
       .overlay(alignment: .top) {
        ...
        .overlay(alignment: .bottom) {
            RoutineBottomSheet(selectedTab: $selectedTab, activityIndex: $activityIndex, viewModel: viewModel)
